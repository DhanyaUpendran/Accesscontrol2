...
export const assignPermissionsToRole = async (req, res) => {
  ...
  role.permissions = permissions; // { permissionKey, scope, startsAt, endsAt }
  await role.save();
...
// 🔹 NEW: Create User with roles, permissions, scope, start/end time
export const createUser = async (req, res) => {
  const { name, email, passwordHash, roles, directPermissions, teamId } = req.body;

  const user = new User({ name, email, passwordHash, roles: [], directPermissions: [] });

  // Assign roles with optional start/end time
  ...
  // Assign direct permissions with scope and optional start/end time
  if (directPermissions && directPermissions.length) {
    for (const p of directPermissions) {
      user.directPermissions.push({
...
// Assign role with optional start/end time
